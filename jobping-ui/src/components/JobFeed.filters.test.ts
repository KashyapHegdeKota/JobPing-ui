...
describe('job feed filtering', () => {
  ...
  it('filters by date', () => {
    ...
    expect(filterJobs(jobs, '', 'All', false, 'All Time', 'All', nowMs)).toHaveLength(3);
    
    // id 3 is 12 hours ago
    expect(filterJobs(jobs, '', 'All', false, 'Past 24 hours', 'All', nowMs).map(j => j.id)).toEqual([3]);
    
    // ids 3, 2, 1 are all within the past week
    expect(filterJobs(jobs, '', 'All', false, 'Past Week', 'All', nowMs).map(j => j.id)).toEqual([3, 2, 1]);
  });

  it('filters by company', () => {
    expect(filterJobs(jobs, '', 'All', false, 'All Time', 'A')).toHaveLength(1);
