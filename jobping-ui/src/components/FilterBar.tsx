...
export interface FilterBarProps { query: string; category: Category; remoteOnly: boolean; dateFilter: DateFilter; resultCount: number; totalCount: number; onQueryChange: (event: ChangeEvent<HTMLInputElement>) => void; onCategoryChange: (category: Category) => void; onRemoteChange: (value: boolean) => void; onDateFilterChange: (value: DateFilter) => void; onClear: () => void; }

export default function FilterBar({ query, category, remoteOnly, dateFilter, resultCount, totalCount, onQueryChange, onCategoryChange, onRemoteChange, onDateFilterChange, onClear }: FilterBarProps) {
  return <div className="border-b border-zinc-800 px-6 py-4 space-y-3 bg-zinc-950/70">
    <div className="flex flex-wrap gap-2 items-center">
      ...
      <select 
        ...
        className="px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-xs font-medium text-zinc-400 focus:outline-none focus:border-cyan-500/50"
      >
